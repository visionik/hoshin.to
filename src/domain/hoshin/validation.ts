...
/**
 * Wizard-ready: exactly five statements with valid prefix + 3–7 words and unique initial order 1–5.
 * Does NOT require connection directions (wizard will set those).
 */
export function validateWizardReady(document: HoshinDocument): ValidationResult {
  const issues: ValidationIssue[] = [];
...
export function canRunWizard(document: HoshinDocument): boolean {
  return validateWizardReady(document).isValid;
}
